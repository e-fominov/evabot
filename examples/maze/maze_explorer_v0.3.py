...
def move_to_cell(robot, direction):
    ...
    log_interval = 0.2  # log every 200ms
    last_log = 0
    cycle = 0
    ...
    while time.time() - start_time < MOVE_TIMEOUT:
        ...
        if t_now - last_log >= log_interval:
            ...
            for sd in perp_dirs:
                ...
                if d_s is not None:
                    parts.append(f"{DIR_NAMES[sd]}={d_s*100:.0f}cm/a={a_s:+.1f}" if a_s is not None else f"{DIR_NAMES[sd]}={d_s*100:.0f}cm")
            if lat_corrections:
                parts.append(f"lat={','.join(f'{DIR_NAMES[k]}:{v*100:+.1f}cm' for k,v in lat_corrections.items())}")
